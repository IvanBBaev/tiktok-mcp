...
| TC-6 | TC-4, TC-5 | `scripts/` (manifest snapshot, readme-sync, env-docs-sync, .env.example gen) + tests | Sync gates wired into `npm run check`. NOTE: the presentation build already ships a hand-written `.env.example` and a hand-written README tools table (between the `GENERATED:TOOLS` markers); TC-6 replaces both with generators + drift gates that must reproduce them byte-for-byte from the live tool registry. | S |
